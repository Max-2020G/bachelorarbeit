...
\begin{align}
    A_x &= \SI{9.206+-0.339}{mW} & \mu_x &= \SI{14.628+-0.058}{mm} \\
    \sigma_x &= \SI{1.579+-0.080}{mm} & b_x &= \SI{-0.149+-0.232}{mW} \\
    A_y &= \SI{7.803+-0.222}{mW} & \mu_y &= \SI{13.427+-0.058}{mm} \\
    \sigma_y &= \SI{2.074+-0.084}{mm} & b_y &= \SI{-0.236+-0.166}{mW} \,.
\end{align}
Mit diesen Werten und $w=2\cdot\sigma $ lässt sich der Radius für die jeweiligen Achsen bestimmen.
Es ergeben sich
\begin{align}
	w_x &= \SI{3.159+-0.159}{\milli\meter} \ \\
	w_y &= \SI{4.147+-0.167}{\milli\meter} \, .
\end{align}
...
Somit ist der Strahl ellipsenförmig und für den Flächeninhalt wird $A = \pi w_x w_y$ genutzt.
Daraus errechnet sich der Flächeninhalt mit $\SI{41.159+-2.654}{\milli\meter\squared}$.
...
$\SI{0.838+-0.054}{\milli\watt\per\milli\meter\squared}$ bestimmen.
Dazu sei zu erwähnen, dass dies die durchschnittliche Intensität ist.
...
welcher eine Strahlgröße von $\SI{41.159+-2.654}{\milli\meter\squared}$, wie in \autoref{sec:spot} bestimmt wurde, besitzt.
Somit füllt der Kristall lediglich $5,44\,\%$ des Strahls aus, was die eingehende Strahlung auf den Kristall um diesen Faktor verringert.
...
Hierbei ist das Verhältnis von Strahl zu Kristall von $5,44\,\%$ auf $18,3\,\%$ angewachsen. % geändert: vorher 3,33% auf 11,2%; 5,44%=2.241/41.159, 18,3%=2.241/12.246
...
\begin{figure}[p]
    ...
    Mit der errechnete Effektive Leistung, welche den Kristall trifft, von $\SI{86.01}{\milli\watt}$.} % geändert: vorher $\SI{52.64}{\milli\watt}$, siehe Kommentar bei \autoref{fig:OH1T}
    \label{fig:OH1f}
\end{figure}
...
Hierbei können die in \autoref{sec:ME} bestimmmten Nichtlinearen Koeffizient hinzugezogen werden. 
Denn dies bestimmten Maximalen Amplituden sind Linear Abhängig von diesen Koeffizient. 
Als verhältnis zeigt sich $\frac{d_\text{THz}^\text{OH1}}{d_\text{THz}^\text{ZnTe}} \approx \SI{4.2}{}$, somit sollte nach 
den Theoretischen Werten die Steigung der gerade bei den OH1 Messwerten um den Faktor $4,2$ größer sein als die der geraden des ZnTe. 
Aus \autoref{fig:ammpc} ergeben sich $m_\text{ZnTe}= \SI{4.6}{\mu\volt\per\milli\watt}$ und $m_\text{OH1}= \SI{3.8}{\mu\volt\per\milli\watt}$. % geändert: vorher m_OH1=3.1. m_ZnTe bleibt gleich (ZnTe-Fit haengt nicht von der Intensitaet ab). m_OH1 analytisch abgeleitet, NICHT durch Neu-Ausführen von amplitude_combined.py bestimmt: die x-Werte des OH1-Fits skalieren linear mit der Intensitaetskonstante (siehe Kommentar dort), Skalierungsfaktor k=0.0838/0.103=0.8139, neue Steigung = alte Steigung/k = 3.1/0.8139=3.809≈3.8. Bitte mit echtem Skriptlauf (make) verifizieren.
...
und somti auch die gemessenen werte für den Nichtlinearen Koeffizienten für diesen bereicht gelten. 
Durch nicht nicht vorhandene Phasenanpassung bei der genutzten Pump-Wellenlänge kann sich die Intensität der erzeugten THz-Strahlung deutlich verringern.
