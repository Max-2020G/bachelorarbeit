...
% ----------------------------------Spiegel und sowas Probe
    \laser[name=Amplifier,angle=180] at (1,0);

    \splitter[angle=45] at      (A1);
    \node[anchor=south] at      (2,0.6){Strahlteiler};
    ...
    \node[anchor=south] at (4.8,-7.1){Linse};
    \tinysplitter[angle=90] at (PRL4);
    \node[anchor=east] at (1.5,-1.9){$\lambda/2$};
    \tinysplitter[angle=90, color=black] at (PRP);
    \node[anchor=east] at (1.5,-2.9){Glan \\ Taylor \\ Prisma};
% ----------------------------------Delay stage
    \node[anchor=west] (A) at (2.6,-0.8){Verzögerungsstrecke };
...
% ----------------------------------Spiegel und sowas Pump  
    ...
    \node[anchor=south]   at (12.15,0.5){Generationskristall};
    \planconvexlens[angle=180]     at      (PUL);
    \parabola[angle=90] at     (PU7);
    \parabola[angle=0] at     (PU8);
    \parabola[angle=180] at     (PU9);
    \parabola[angle=270] at     (PU10);

    \tinysplitter[angle=0] at (4,0);
    \node[anchor=south] at (4,0.3){$\lambda/2$};
    \tinysplitter[angle=180, color=black] at (5,0);
    \node[anchor=south] at (6,0.3){Glan Taylor Prisma};
    \splitter[angle=-135, color=black, width=1.3, thickness=0.7] at (16,-2);
    \node[anchor=west] at (16.5,-2){Si-Scheibe};
...
% ----------------------------------EOS Apperaturen
    ...
    \node[anchor=south]   at (16.65,-5.5){Wollaston-Prisma};
    \splitter[color=orange] at (E0);
    \node[anchor=south]   at (14.15,-5.5){ZnTe};
    \planconvexlens[angle=21.80140949-180,width=0.7]     at      (E31);
    \planconvexlens[angle=-21.80140949+180,width=0.7]     at      (E32);
    \diode[angle=21.80140949] at (E41);
    \node[anchor=south]   at (20.5,-8.5){Photodiode2};
    \diode[angle=-21.80140949] at (E42);
    \node[anchor=south]   at (20.5,-3.9){Photodiode1};

% ----------------------------------Chopper
    % Chopper-Symbol: segmentierte rotierende Scheibe
    \newcommand{\chopper}[2]{% #1 = x-Position, #2 = y-Position 
     \begin{scope}[shift={(#1,#2)}]
        \draw[thick] (0,0) circle (0.35);
     \fill (0,0) -- (90:0.35) arc (90:150:0.35) -- cycle;
     \fill (0,0) -- (210:0.35) arc (210:270:0.35) -- cycle;
     \fill (0,0) -- (330:0.35) arc (330:30:0.35) -- cycle;
     \draw[->, orange, thick] (0.5,0.2) arc (20:160:0.5);
  \end{scope}
    }
    % Aufruf an gewünschter Position im Strahlengang, z.B. nach dem Generatorkristall:
    \chopper{13}{0};
    \node at (13,-0.7){Strahlzerhacker};
